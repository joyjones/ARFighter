...
class Character: SCNNode {
    ...
    private var _bornPos: float3?
    
    // Physics
    private var characterCollisionShape: SCNPhysicsShape?
    private var collisionShapeOffsetFromModel = float3(0)
    private var downwardAcceleration: Float = 0
    var physicsWorld: SCNPhysicsWorld?
    
    init(model: SceneModel, ModelFile file: String) {
    ...
    func update(updateAtTime time: TimeInterval, camera: ARCamera?){
        ...
        parentCamera = camera
        self.simdPosition += movingDirection * movingSpeed
        if simd_length(movingDirection) > 0 {
    ...
    func move(dirShift: Float, dirForward: Float) {
        ...
        if dirShift == 0 && dirForward == 0{
            movingDirection = float3(0, 0, 0)
            playAnimation(key: "idle")
            return
        }
        let len = sqrt(dirShift * dirShift + dirForward * dirForward)
        let mat = SCNMatrix4(parentCamera.transform)
        let dir = float3(-1 * mat.m31, -1 * mat.m32, -1 * mat.m33)
        let forward = simd_normalize(float3(dir.x, 0, dir.z))
        let left = simd_cross(dir, float3(0, 1, 0))
